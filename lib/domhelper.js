...
import { Selector, List } from './types'
import extendedPrototype from './extendedPrototype';
import polyfill from './polyfill';

export const getFragment = () => {
    return document.createDocumentFragment();
};
export const getEmptyNodeList = () => {
    return getFragment().querySelectorAll('*');
};
...
export const map = function(
        ...
        if(container.length===1){
            ...
            list = toNodeList(container);
            list = rebuildFromHookContainer(list._$hookcontainer);
            delete list._$hookcontainer;
        }
        return list;
};
 ...
 * @return {[type]} [description]
 */
export const toNodeList = function(list : Array<NodeList>){

    var hookContainer : Array = [],
        docFragment = getFragment(),
        hookNode, type, node;
    ...
    childNodes._$hookcontainer = hookContainer
    return childNodes;
};
...
export const rebuildFromHookContainer = function(hookcontainer){
    ...
    return each(document.querySelectorAll(".__lyadom__"), (_, domnode) => {
        removeClass(domnode, new String('__lyadom__'));
    });
}

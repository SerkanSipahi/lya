...
export const map = function(
        ...
        for(let node of nodes){
            // wenn callback nicht gleich null liefert dann container.push
            let list: NodeList = callback.bind(null)(node, i);
            container.push(list);
        }
 ...
 * @return {[type]} [description]
 */
export const toNodeList = function(list : List<NodeList>){ //List<Nodelist> kann auch ein array sein

    var hookContainer : Array = [],
        container : Array = [],
        docFragment = document.createDocumentFragment(),
        DOMNode, hookNode, type;

    /*
     * Auch eine andere Variante ist möglich und auch wohl die
     * sichere und schnellere. Unique Platzhalter DomNodes erstellen
     * und diese dann mit den echten DomNodes tauschen( replceChild() )
     */
    ...
    // > refactor: merge to first for iterator
    // keep positions
    for(let node of container){
        ...
        hookContainer.push([DOMNode, hookNode, type]);
    }

    // > refactor: merge to first for iterator
    for(let [node] of hookContainer){
...
export const rebuild = function(hookContainer : Array){
